...
def pixel_based(path, folder_pixel_res, folder_subsets, pixel_res_path, ras_extension):
    pixel_res_path = os.path.join(path, pixel_res_path)
    if not os.path.isdir(path + folder_pixel_res):
        os.makedirs(pixel_res_path)
    ...
    for name in glob.glob(path + folder_subsets + str(ras_extension)):
        pix_subset_list.append(name)
    pix_subset_list = [w.replace('\\', '/') for w in pix_subset_list]
    pix_subset_name = [w[len(path + folder_subsets):-(len(ras_extension) - 1)] for w in pix_subset_list]
    ...
    for i, subset in enumerate(pix_subset_list):
        ...
        with rio.open(pix_subset_list[i]) as src:
            ...
            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str('_NDVI_sb') + str('.tif'), 'w',
                          **ras_meta) as dst:
                dst.write(ndvi_pixel[i], 1)
                # print(pix_subset_name[i] + str("_NDVI"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str('_CCCI_sb') + str('.tif'), 'w',
                          **ras_meta) as dst:
                dst.write(ccci_pixel[i], 1)
                # print(pix_subset_name[i] + str("_CCCI"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str('_GARI_sb') + str('.tif'), 'w',
                          **ras_meta) as dst:
                dst.write(gari_pixel[i], 1)
                # print(pix_subset_name[i] + str("_GARI"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str('_NDRE_sb') + str('.tif'), 'w',
                          **ras_meta) as dst:
                dst.write(ndre_pixel[i], 1)
                # print(pix_subset_name[i] + str("_NDRE"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str("_NDMI_sb") + str(".tif"), "w",
                          **ras_meta) as dst:
                dst.write(ndmi_pixel[i], 1)
                # print(pix_subset_name[i], str("_NDMI"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str("_NDWI_sb") + str(".tif"), "w",
                          **ras_meta) as dst:
                dst.write(ndwi_pixel[i], 1)
                # print(pix_subset_name[i], str("_NDWI"), str(".tif"), str("created"))

            with rio.open(pixel_res_path + str(pix_subset_name[i]) + str("_CVI_sb") + str(".tif"), "w",
                          **ras_meta) as dst:
