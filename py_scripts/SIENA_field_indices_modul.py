...
def subset_import(subset_list, outpath_subsets, ras_extension, inpath, shp_extension):
    ...
    for i, name in enumerate(shp_names):
        for name in glob.glob(str(outpath_subsets) + str("*" + str(shp_names[i])) + ras_extension):
...
def indi_test(outpath_indices_csv, subset_list, outpath_subsets, ras_extension, inpath, shp_extension,
                        csv_extension):
    subset_list = subset_import(subset_list, outpath_subsets, ras_extension, inpath, shp_extension)
    shp_names = SIENA_data_import_modul.shp_names(inpath, shp_extension)
    for i, shp_names in enumerate(shp_names):
        for i, subset in enumerate(subset_list):
            ...
            if (i + 1) == (len(subset_list)):
                with open(outpath_indices_csv + str("_test_") + csv_extension[1:], 'w',
...
# def indices_field_based(outpath_indices_csv, subset_list, outpath_subsets, ras_extension, inpath, shp_extension,
#                         csv_extension):
#     subset_list = subset_import(subset_list, outpath_subsets, ras_extension, inpath, shp_extension)
#     for i, subset in enumerate(subset_list):
#         with rio.open(subset_list[i]) as src:
